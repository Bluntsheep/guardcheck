...
import GuardTable from "@/app/components/viewcvComponent";
import Cvviewer from "@/app/components/cvviewer/cvviewer";
...
const CvsPage = () => {
  ...
  const [refreshCVs, setRefreshCVs] = useState(false); // New state to trigger a re-fetch
  ...
  const handleChange = (selectedProvince) => {
    ...
    setCurrentCV(null); // Reset currentCV when province changes
  };
  ...
  const handleDeleteCV = async (cvId) => {
    ...
    if (window.confirm("Are you sure you want to delete this CV?")) {
      try {
        const response = await fetch(`/api/cvUpload?id=${cvId}`, {
          method: "DELETE",
        });

        if (response.ok) {
          setRefreshCVs((prev) => !prev);
          console.log(`CV with guardcv_id ${cvId} deleted successfully.`);
        } else {
