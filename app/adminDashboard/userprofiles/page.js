...
const AdminDashboard = () => {
  ...
  const [updatingIds, setUpdatingIds] = useState(new Set()); // Track which accounts are being updated
  ...
  const handleStatusToggle = async (account) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to ${action} the account for ${account.company_name} (${account.d_user})?`
      )
    ) {
      return;
    }
    ...
    setUpdatingIds((prev) => new Set([...prev, accountId]));

    try {
      // Updated to use PATCH method and the accounts endpoint
      const response = await fetch(`/api/admin/accounts`, {
        method: "PATCH",
        headers: {
      ...
      if (data.success) {
        ...
        setAccountsData((prev) =>
          prev.map((item) =>
            item.id === accountId ? { ...item, active: newStatus } : item
          )
        );

        console.log(`Account successfully ${action}d:`, data.data);

        // Show success message using the response message
        alert(
          data.message ||
            `Account for ${account.company_name} has been successfully ${action}d.`
        );
      } else {
        console.error("Status update failed:", data.error);
        alert(`Failed to ${action} account: ${data.error}`);
      }
    } catch (error) {
      console.error("Error during status update:", error);
      alert(
        `An error occurred while updating the account status. Please try again.`
      );
    } finally {
      setUpdatingIds((prev) => {
        ...
        <div className="grid grid-cols-1 gap-2 text-sm">
          ...
          <div className="pt-3 border-t border-gray-200 flex justify-end">
            <button
              ...
              disabled={isUpdating}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm ${
                isActive
                      ...
                      <tr
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            onClick={() => handleStatusToggle(account)}
                            disabled={isUpdating}
                            className={`flex items-center gap-1 px-3 py-1 rounded hover:opacity-80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm mx-auto ${
                              isActive
